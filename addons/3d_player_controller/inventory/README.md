...
| `HUD/Hint` | Shows `item_used` and `item_dropped` as they fire; it only signals, the game applies the effect. |
...
|---|---|---|
| `Inventory` (`scenes/inventory.tscn`, already inside `player.tscn`) | Child of the Player | `slots_per_tab` (20), `persist` (off by default so tests and bare Players leave the disk alone; turn it on in your game scene), `save_path` (`user://inventory.tres`), `next_weapon_action` / `last_weapon_action` (`next_weapon` / `last_weapon`) |
...
| `ItemPickup` (`scenes/item_pickup.tscn`) | In your level, on the ground | `item`, `count`, `show_icon` (the icon floats over the spot; add your own mesh and turn it off), `take_action` (`action`) |
| `Equipment` walk-over pickups | Unchanged: an `Equipment` scene with its `PlayerDetection` area is taken on contact and appears on the Equipment tab | Make it a `.tscn` so the save can bring it back; equipment placed inline in a level cannot be re-created and is not saved | A piece is named to a peer by its own scene when it has one (a `.tscn` carries its script and settings), otherwise by the path of the world pickup it came from, which is what a model file with the `Equipment` script put on in the level needs: the peer duplicates that pickup. A piece placed inline that was never a pickup stays local. Only a piece with a scene of its own can be thrown, since what lands is an instance of that scene. A piece equipped on a Player takes that Player's multiplayer authority, attachment included, so its own `_ready` runs as the owner's on a client and not as a puppet's copy.

**Grid.** Every tab is `slots_per_tab` cells; Materials stack to their item's `max_stack` (999 for the demo ore), Key Items to 1. Confirm on a stack lifts it onto the cursor (it follows the mouse, or the focused cell on a pad or keyboard); Confirm on an empty cell moves it there, on the same item merges up to the limit, on anything else swaps. Cancel (B / Esc) puts a held stack back, or goes Back. Use emits `item_used(item, count)` and takes one from a consumable; Drop takes one and spawns an `ItemPickup` a metre in front of the Player, emitting `item_dropped(item, count, pickup)`. A drop lands in every peer's world under one name, `Dropped_<peer id>_<n>` (an `Inventory` RPC any peer may send), so a pickup taken by one peer vanishes for all; an item with no resource path cannot travel and drops on the dropping peer alone. The Equipment tab lists what is on the skeleton and in the backpack: Confirm (or the Equip button) equips a stowed weapon or stows an equipped one, Drop puts its scene back in the world. `items_changed` fires after every stack change, `equipment_changed` after every equip.

**Pickups.** Standing in a pickup's `PlayerDetection` shows the `ActionPrompt` and reads the Action button as "Pick Up"; Action (`take_action`, ignored while a menu is up or the chat has the keyboard) puts what fits in the inventory and, once it is empty, frees the pickup on every peer (`_vanish`, an RPC the taker sends). Equipment items are picked up by instancing their `equipment_scene` and handing it to `equip_pickup`, exactly as a walk-over pickup does: the attach is the inventory's (a `BoneAttachment3D` on the named bone holding a copy with the scene's hand offsets, world collision off, `Hitbox` kept; the instance is in the tree for the moment it equips so the offsets reach the copy; a save re-equips the same way); a second of the same type on the same bone is refused, so it stays on the ground.
...
**Saving.** `save()` writes an `InventorySave` (every stack with its tab and slot, every scene-based weapon with whether it was equipped) to `save_path`; `load_save()` reads it back and re-equips. With `persist` on, `save()` runs once at the end of any frame that changed something and `load_save()` runs once the Player is ready, since the skeleton and the abilities node a save re-equips and refills are the Player's `@onready` nodes. A saved piece whose scene cannot be loaded is skipped without shifting the equipped flags of the rest. Over the network the equipment of every other peer's copy of the Player follows the authority's (`_sync_equipment`); their inventories never save. The file is a readable `.tres`: change a count in a text editor and it is so.
