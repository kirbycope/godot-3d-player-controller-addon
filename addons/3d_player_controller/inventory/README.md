...
| `Equipment` walk-over pickups | Unchanged: an `Equipment` scene with its `PlayerDetection` area is taken on contact and appears on the Equipment tab | Make it a `.tscn` so the save can bring it back; equipment placed inline in a level cannot be re-created and is not saved | Any scene the loader knows can travel, an imported `.fbx` or `.glb` model as much as a `.tscn`; a piece placed inline in a level has no path and stays local.
