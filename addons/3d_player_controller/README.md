...
- **`TalkingNpc`** (`scenes/talking_npc.tscn`): somebody to talk to. Get within arm's length and look at them, and the `ActionPrompt` offers `Talk` (`prompt_label`); Action opens their `dialogue` in the Player's `DialogueScreen` with `display_name` as the speaker, and they face the talker until it ends (`talked_to` fires). Left with no `player` they stand; given one they follow like any `FollowerNpc`. The walk and run blend replicates.
