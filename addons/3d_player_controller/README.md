...
- Optional and self-contained: the lobby scenes reach Steam only through `Engine.has_singleton("Steam")` and the `/root/Steamworks` autoload when present, and read lobby names, members and avatars only while that autoload reports a signed-in `steam_id` (the extension loaded with no client behind it, as on CI, errors on every lobby call), and ship with plain `TextureRect`/`Label` nodes and Kenney icons, so the addon has no dependency on GodotSteam or GodotSteamKit and still exports to web. Without Steam the UI reports "Steam unavailable" and disables its buttons.
