...
- **Split Settings Menu**:
  ...
  - **Scheme** (in Video settings, `PlayerSettingsResource.control_scheme_name`): `Default` plus every registered layout, which here is `Zelda` and `Platformer`, plus whatever a game registers, such as the gta addon's `GTA`; there is no `Default` row; `Default` keeps whatever the scene set on `Player.control_scheme`.
