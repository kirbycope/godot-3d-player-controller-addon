...
### 14. Game-Style Systems (`WantedLevel`, `MuzzleFlash`, `TreasureChest`, `Bonfire`, `Vitals`, `CraftingStation`)
...
- **`MuzzleFlash`** (`scenes/muzzle_flash.tscn`): a flash and a light for any `Firearm` or shooting `EnemyNpc`; sit it under the Muzzle marker and wire `fired` to `flash`. A full `Firearm` is a muzzle marker, a fire timer, a laser sight if wanted, this flash and a pickup area, and an `AmmoItem` in the inventory reloads it.
...
| `assets/quaternius/fantasy_props/` (the chest, the scroll, the bronze sword and the workbench, with their textures) and `stylized_nature/` (the mushroom and three rocks) from the Fantasy Props MegaKit and the Stylized Nature MegaKit | [Quaternius](https://quaternius.com/) | CC0 1.0 (`License.txt` in each folder) |
| `assets/icons/` (`star.svg`, the wanted level's star) | Drawn for this addon | CC0 |
