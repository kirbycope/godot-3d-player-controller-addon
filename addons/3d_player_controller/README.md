...
- **Split Settings Menu**:
  ...
  - **Controls Settings** (`ControlsSettings`, `scenes/controls_settings.tscn`): the two rows below, which used to sit among the video options. Nothing here touches the viewport; both rows write `PlayerSettingsResource` and take effect on the Player at once.
