...
- **The scene is a gallery**: both scripts are tool scripts. In the editor an `AbilityEntry` takes its ability's name and puts the ability's icon, its name and a live instance of its casting and impact VFX under itself, and the library lays its entries out in a row `gallery_spacing` apart, so opening `ability_library.tscn` shows every spell the game has. The previews carry no owner, so they are never saved, and they are not made in the running game.
- **Fire it in the editor**: in a test scene with a Player and an enemy, set the library's `test_caster` and `test_target` to them; every entry's inspector then has two buttons, casting the ability from one to the other and back (`AbilityEntry.preview_cast(caster, target)`): the channeling VFX and sound on the caster for the cast time, the casting phase, carried to the target as a bolt over `projectile_speed` when the ability has one, then the impact at the target. It is the look and sound of the cast; nothing takes damage.
