...
- **In the editor**: both scripts are tool scripts. An `AbilityEntry` is drawn with the addon's spell icon in the scene tree, takes its ability's name when it is still called by the default, and shows the ability resource in its inspector.
