...
- **Control schemes** (`PlayerControls.ControlScheme`, picked with `Player.control_scheme`): which game's pad the face buttons are laid out like, and what Focus does. `ZELDA`, the scene's own, is A Action, B Sprint, X Attack, Y Jump, with Focus locking on to a target. `GTA` is A Sprint, B Attack, X Jump, Y Action, and Focus is a free over-the-shoulder aim with anything in hand (`Player.lock_on_enabled()` is false, so `Focus` acquires nobody and the camera keeps the aim FOV and shoulder offset for as long as focus is held instead of swinging behind the Player). The keyboard keys, the shoulders, the triggers, the sticks and the d-pad are the same in both. `PlayerControls.SCHEME_SLOTS` is the table, `apply_control_scheme()` swaps the slots live: each face button comes off the action it stood for, goes on to its new one, the resting labels follow, and a project that bound an action for itself in `project.godot` is left alone. The Video settings menu has a `Scheme` option (`Default`, `Zelda`, `GTA`; `PlayerSettingsResource.control_scheme_index`) that overrides the scene's export for the player who picked it.
...
- **Split Settings Menu**:
  ...
  - **Scheme** (in Video settings, `PlayerSettingsResource.control_scheme_index`): `Default`, `Zelda` or `GTA`, the control scheme of section 5; `Default` keeps whatever the scene set on `Player.control_scheme`.
  - **Controls** (in Video settings, `PlayerSettingsResource.hud_mode`): `Auto`, `Shown` or `Hidden`. `Auto`, the default, draws the HUD only while the device in hand is a touchscreen, so a keyboard or a pad plays without it; the Player follows `Controls.input_type_changed`. `Player.apply_hud_visibility()` puts the rule back after a game or a menu has hidden the HUD for a while, so nothing calls `controls.show()` directly.
...
- **`TalkingNpc`** (`scenes/talking_npc.tscn`): somebody to talk to. Look at them and the `ActionPrompt` offers `Talk` (`prompt_label`); Action opens their `dialogue` in the Player's `DialogueScreen` with `display_name` as the speaker, and they face the talker until it ends (`talked_to` fires). Left with no `player` they stand; given one they follow like any `FollowerNpc`. The walk and run blend replicates.
