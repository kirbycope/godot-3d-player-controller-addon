...
- **Split Settings Menu**:
  ...
  - **Scheme** (in Video settings, `PlayerSettingsResource.control_scheme_name`): `Default` plus every registered layout, which here is `Zelda`, `GTA` and `Platformer`, the control schemes of section 5; `Default` keeps whatever the scene set on `Player.control_scheme`.
