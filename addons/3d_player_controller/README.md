...
- **How much noise the Player makes** (`PlayerNoise`, `scripts/player_noise.gd`, a child of the Player): a
  reading from 0 to 1 that anything can listen to. Moving sets a floor, with the tier coming from what the
  Player is doing (crouched, sprinting, or neither) and the pace within it from how fast they are actually
  travelling, since movement here is root motion rather than a speed export. `is_stealthed` quiets it and
  swimming softens it. One-off noises spike the reading and fall back: a landing, a slide, a swing that lands,
...
- **`NoiseMeter`** (`scenes/noise_meter.tscn`): the readout, after Breath of the Wild's. A round dark badge
  ...
  screen and the distance the noise carries are the same number.
