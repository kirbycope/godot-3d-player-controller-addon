...
| `assets/click_to_move/arrow_up.png` and `assets/shaders/click_to_move.gdshader` (the click-to-move marker: three spheres wrapped in a scrolling arrow) | Brought over from the Guild Wars Heroes Island project (`godot-guild-wars`); the arrow's own source is not recorded - fill in | Not recorded - fill in |
